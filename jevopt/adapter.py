...
def _score_one(instance: dict, questions: dict, options: list[str]) -> dict:
    state = instance["state"]
    good = set(instance["acceptable"])
    try:
        body = client.ask(state, questions)
        answer = body["answers"][grammar.QUESTION]
    ...
    probs = {o: float(answer.get("probabilities", {}).get(o, 0.0)) for o in options}
    best_good = max((probs[o] for o in good), default=0.0)
    best_bad = max((probs[o] for o in options if o not in good), default=0.0)
...
class JevAdapter:
    """Implements the GEPAAdapter protocol for a Task's choice question."""
        ...
        for component in components_to_update:
            ...
            for trace in eval_batch.trajectories or []:
                ...
                records.append({
                    ...
                    "model_chose": chosen,
                    "p_this_option": round(probs.get(option, 0.0), 3),
